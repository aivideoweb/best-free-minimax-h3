...
Reader routes: homepage five-second prompts are distinct from advanced multi-reference examples. Label advanced duration/input requirements and English-only destinations. Keep the six primary-tool comparison links pointed at the matching homepage workflow; offer standalone prompts as different subjects. `docs/prompt-library_zh.md` summarizes one verified example per category, not all 84 translations. Update its inputs, duration and review notes against the linked English text. `docs/product-reference*.md` explains concept versus endpoint-guided work; `docs/commerce-three-shots*.md` distinguishes editing one generation from making an optional extra take.
